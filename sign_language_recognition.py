...
# applied tqdm module to for loop
...
# output                                       train image   label      split
...
model=Sequential()
# add pretrained models tp Sequential model
# EfficientnetB0 pretrained model is used here ,other models can be used
...
# now add dense layer as output
model.add(layers.Dense(1))
# For some tensorflow version , build model is necessary
...
# save and run
# pc should be connected to internet to download pretrained weight
# GPU is used to train the models

# compile model
# different optimizers and loss function can be used to increase the accuracy
model.compile(optimizer="adam",loss="mae",metrics=["mae"])

# I will create a checkpoint to save best accuracy model
...
# Here you can select either "min" or "auto"
...
# patience=X
# reduce lr after X when accuracy does not improve
...
# Before training I will delete the image_array and label_array to increase the ram memory
...
# mae: Is on X_train
# val_mae: X_test
# If val_mae is reducing that means model is improving.
